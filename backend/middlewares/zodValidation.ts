...
import { LoginInput, SignupInput, UpdateInput } from "../types";
...
const userSignUpSchema = zod.object({
    ...
    password: zod.string().min(7, { message: "Password must be at least 7 characters" })
})
...
const userUpdateSchema = zod.object({
    ...
    password: zod.string().min(7, { message: "Password must be at least 7 characters" }).optional()
...
const transferMoneySchema = zod.object({
    ...
    amount: zod.number({ message: "amount is missing" }).min(1, { message: "Minimum amount should be 1." }),
})
...
export const transferMoneyValidation = (req: Request, res: Response, next: NextFunction) => {
